...
const ComisionWithdraw: React.FC = () => {
  ...
  const fetchUserBalance = async (userId: string) => {
    ...
    if (userDoc.exists()) {
      ...
      setBalance(userData?.referralBalance || 0); 
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (parseFloat(withdrawAmount) > 1000000) {
      setError('Maximum withdrawal amount is 1,000,000.');
      return;
    }
    ...
    try {
      ...
      await updateDoc(userDocRef, {
        referralBalance: balance - parseFloat(withdrawAmount),
      });
...
export default ComisionWithdraw;
